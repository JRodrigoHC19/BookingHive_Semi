...
class HotelController extends Controller
{
    public function index($ruc_hotel ,$id_user, Request $request)
    {
        if ($ruc_hotel == 0){
            ...
            $ruc_hotel = Hotel::where('titulo', $titulo)->first()->id;
        }

        $hoteles_full = Hotel::get();
        // OBTIENE LOS DATOS DEL HOTEL 
        $hotel = Hotel::find($ruc_hotel);

        // VERIFICA SI EL USUARIO INGRESO AL HOTEL Y SI EL USUARIO NO ES DE TIPO [CUENTA HOTEL]
        if (Visit::where('user_id', $id_user)->where('ruc', $ruc_hotel)->doesntExist() && User::find($id_user)->rol != 'M')
        {
            $visita = new Visit();
        ...
        $resenas = Resena::where('ruc_hotel', $hotel->id )->get();
