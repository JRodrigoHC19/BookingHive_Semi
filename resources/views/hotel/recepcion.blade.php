...
<body class="u-body">
    ...
    <div id="app">
      ...
      <form class="card" action="{{ route('concretar-reservacion') }}" method="POST">
        @csrf
        <div class="row">
      ...
      <div>
        ...
        @foreach ($resenas as $resena )
          <p>
            COMENTARIO: {{$resena->msg}} FECHA_ENVIADA: {{$resena->created_at}} USUARIO: {{user::where('email',$resena->email)->first()->name}}
          </p>
        @endforeach
      </div>
      ...
      @auth
        @if (auth()->user()->rol == 'R')
          <!-- REGISTRAR RESEÑA -->
          <form class="card" action="{{ route('comentar') }}" method="POST">
            @csrf
            <!-- VALORACIONES -->
      ...
      <div>
        @foreach ($fotos as $figura)
          <img width="180px" height="100px" src="{{ asset('storage/fotos/'.$figura->nombre) }}" alt="figurita">
        @endforeach
      </div>
    </div>
